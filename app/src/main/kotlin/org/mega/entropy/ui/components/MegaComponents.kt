...
/**
 * A numeric index/account field that defaults to "0". Filtering the raw
 * String value alone doesn't fix typing over that default: depending on
 * which side of the "0" the cursor lands on, a typed "1" becomes "10" or
 * "01" in the raw text before filtering ever runs, and stripping leading
 * zeros afterward can't recover which digit the user meant once both are
 * already concatenated. Selecting the whole field on focus makes the
 * first keystroke replace "0" outright instead of merging with it.
 */
@Composable
fun MegaIndexField(
    value: String,
    ...
    OutlinedTextField(
        value = fieldValue,
        onValueChange = { new ->
            val digits = new.text.filter { it.isDigit() }.trimStart('0').ifEmpty { "0" }
